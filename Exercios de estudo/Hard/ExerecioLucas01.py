...
if tird_date is not None and tird_date < data_atual:
    data3_dif = abs(datetime.strptime(tird_date, "%Y-%m-%dT%H:%M:%S") - datetime.strptime(data_atual, "%Y-%m-%dT%H:%M:%S"))
else:
    data3_dif = None
...
if data_dif == data1_dif and data1_dif is not None:
    print("A data mais perto da data atual é NewestPefinDate com " + str(data_dif) + " de diferença")
else:
    if data_dif == data2_dif and data2_dif is not None:
        print("A data mais perto da data atual é NewestProtestsDate com " + str(data_dif) + " de diferença")
    else:
        if data_dif == data3_dif and data3_dif is not None:
            print("A data mais perto da data atual é NewestLawsuitsDate com " + str(data_dif) + " de diferença")
